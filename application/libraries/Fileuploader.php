...
class Fileuploader
{
	...
	function upload_avatar($form_field_name,$user_id){
		$upload_path = PROFILE_ATTACHMENT_UPLOAD_PATH."/".$user_id;
	...
	function upload_offer_attachments($form_field_name,$user_id){
		$upload_path = OFFER_ATTACHMENT_UPLOAD_PATH."/".$user_id;
		if(!is_dir($upload_path)) mkdir($upload_path,0777,true);
		$config['upload_path'] = $upload_path; // after baseurl, ./uploads/
		$config['allowed_types'] = 'jpg|gif|jpeg|png';
		$config['encrypt_name']=TRUE;
	...
	function upload_profile_attachments($form_field_name,$user_id){
		$upload_path = PROFILE_ATTACHMENT_UPLOAD_PATH."/".$user_id;
		if(!is_dir($upload_path)) mkdir($upload_path,0777,true);
		$config['upload_path'] = $upload_path; // after baseurl, ./uploads/
		$config['allowed_types'] = 'jpg|gif|jpeg|png';
